...
import { PanelItem, PanelUI } from "./uicomponents/panelui"
import { RawUI } from "./uicomponents/rawui"
import { TextBtnUI } from "./uicomponents/text-btnui"
import { UIComponent, h } from "./uicomponents/uicomponent"
...
class TariUI {
  ...
  process_requests() {
    ...
    const callback = this._requests.pop()
    callback(ui)
    if (ui.vaild) {
      ui.unlock()
    }
...
class UIInstance {
  ...
  PanelUI(
    ...
    return new PanelUI(max_show_count, search, get_list, press_enter, close)
  }
